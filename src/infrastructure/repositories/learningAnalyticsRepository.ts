...
export class LearningAnalyticsRepository implements ILearningAnalyticsRepository {
  ...
  constructor(private firestore: Firestore) {
    this.progressRepository = new ProgressRepository(firestore, auth);
  }
   ...
   * 進捗データを日付ごとに集計する
   */
  private aggregateProgressByDate(
  ...
  private createChartData(
    ...
    groupedData.forEach((dataPoints, groupKey) => {
      ...
      // 学習頻度を計算（学習日数 / 総日数）
      const frequency = dataPoints.length > 0 ? studyDaysCount / dataPoints.length : 0;
  ...
  private calculateSummary(
    ...
    // 平均学習時間（分）
    const averageStudyTime = studyDates.length > 0 ? totalStudyTime / studyDates.length : 0;

    // 平均満足度
    const averageSatisfaction = satisfactionCount > 0 ? totalSatisfaction / satisfactionCount : 0;

    // 平均ページ数（1日あたり）
    const averagePagesPerDay = studyDates.length > 0 ? totalPages / studyDates.length : 0;

    // 学習頻度（学習日数 / 総日数）
    const studyFrequency = totalDays > 0 ? studyDates.length / totalDays : 0;
   ...
   * 連続学習日数を計算する
   */
  private calculateStudyStreaks(dailyData: Map<string, LearningAnalyticsData>): {
  ...
  async analyzeProgress(
    ...
    try {
      ...
      const progressData = await this.progressRepository.getByDateRange(
        startDate,
        endDate,
        subjectId
      );
