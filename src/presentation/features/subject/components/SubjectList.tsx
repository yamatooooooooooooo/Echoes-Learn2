...
import {
  ...
  SubjectCreateInput,
  SubjectUpdateInput,
} from '../../../../domain/models/SubjectModel';
...
export const SubjectList: React.FC<SubjectListProps> = ({ formatDate }) => {
  ...
  const [selectedSubjectForProgress, setSelectedSubjectForProgress] = useState<Subject | null>(
    null
  );
      ...
      try {
        ...
        const updatePromises = subjectList.map((subject: Subject) => {
          ...
          const currentPriority =
            subject.priority === 'high' ? 3 : subject.priority === 'medium' ? 2 : 1;

          if (newPriority !== currentPriority) {
            // 優先順位が変わった場合のみ更新
            const priorityString =
              newPriority === 3 ? 'high' : newPriority === 2 ? 'medium' : 'low';
    ...
    async (updatedSubject: Subject) => {
      try {
        ...
        if (autoPriority) {
          ...
          const currentPriority =
            updatedSubject.priority === 'high' ? 3 : updatedSubject.priority === 'medium' ? 2 : 1;
          ...
          if (newPriority !== currentPriority) {
            ...
            setSubjects((prev) =>
              prev.map((s) =>
                s.id === updatedSubject.id ? { ...updatedSubject, priority: priorityString } : s
              )
            );
            return;
          }
  ...
  // 自動優先順位設定のトグル - メモ化
  const handleAutoPriorityToggle = useCallback(
  ...
  const handleFormSubmit = useCallback(
    async (formData: SubjectCreateInput | SubjectUpdateInput) => {
      try {
        const userId = auth.currentUser?.uid || 'current-user';
        ...
        if (autoPriority) {
          ...
          finalSubjectData.priority =
            priorityValue === 3 ? 'high' : priorityValue === 2 ? 'medium' : 'low';
        }
  ...
  return (
    ...
    >
      ...
      >
        <SubjectListHeader onAddSubject={handleAddSubject} totalSubjects={subjects.length} />
      </Box>
                    ...
                    <Grid item xs={12}>
                      <Paper
                        ...
                        sx={{
                          ...
                          borderRadius: 2,
                          bgcolor:
                            theme.palette.mode === 'dark' ? 'rgba(30, 30, 30, 0.7)' : 'white',
                          display: 'flex',
                          flexDirection: 'column',
                          alignItems: 'center',
                          justifyContent: 'center',
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            {subjectToDelete ? (
              <>
                科目「<strong>{subjectToDelete.name}</strong>」を削除します。
                <br />
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={handleCancelDelete} disabled={isDeleting} sx={{ borderRadius: 2 }}>
            キャンセル
          </Button>
          <Button
