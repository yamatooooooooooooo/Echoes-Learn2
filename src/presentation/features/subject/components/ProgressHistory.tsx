        ...
        <ListItem
          secondaryAction={
            <Stack direction="row" spacing={0.5}>
              <Tooltip title="詳細">
                <IconButton edge="end" size="small" onClick={() => onShowDetails(progress)}>
                  <InfoIcon fontSize="small" />
                </IconButton>
              </Tooltip>
              <Tooltip title="編集">
                <IconButton edge="end" size="small" onClick={() => onEdit(progress)}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                {progress.reportProgress && (
                  <Tooltip title="レポート進捗あり">
                    <AssignmentIcon fontSize="small" color="secondary" sx={{ ml: 1 }} />
                  </Tooltip>
                )}
  ...
  ({
    ...
    return (
      <Box>
        ...
        >
          <Typography variant="subtitle2">{formatDateHeading(dateStr)}</Typography>
        </Box>
...
export const ProgressHistory: React.FC<ProgressHistoryProps> = ({
  ...
  const handleFilterChange = useCallback(
    (_event: React.MouseEvent<HTMLElement>, newFilter: FilterType | null) => {
      if (newFilter !== null) {
        setFilter(newFilter);
      }
    },
