...
export function calculatePriority(
  subjectOrDate: Subject | Date | null,
  currentPage?: number,
  totalPages?: number
  ...
  if (subjectOrDate && typeof subjectOrDate === 'object' && 'id' in subjectOrDate) {
    ...
    // 試験日までの日数がない場合
    if (!subject.examDate) {
      return subject.importance === 'high' ? 'high' : 'low';
    }
  ...
  // examDate, currentPage, totalPagesが渡された場合
    ...
    const baseScore = (remainingPercent / daysRemaining) * 10;
...
export const getPriorityColor = (priority?: 'high' | 'medium' | 'low'): string => {
  switch (priority) {
    case 'high':
      return '#f44336'; // 赤色（高優先）
    case 'medium':
      return '#ff9800'; // オレンジ色（中優先）
    case 'low':
      return '#4caf50'; // 緑色（低優先）
    default:
      return '#9e9e9e'; // グレー（未設定）
  }
};
