...
import { touchFriendlyStyles, responsiveTextStyles } from '../../../styles/responsiveStyles';
...
export const LearningAnalyticsCard: React.FC<LearningAnalyticsCardProps> = ({
  ...
  const timeOfDayEfficiency = useMemo(() => {
    return TIME_OF_DAY.map((timeSlot, index) => {
      ...
      const averageEfficiency =
        sessionsInTimeSlot.length > 0
          ? Math.round(
              sessionsInTimeSlot.reduce((sum, session) => sum + session.efficiency, 0) /
                sessionsInTimeSlot.length
            )
          : 0;
  ...
  const recommendations = useMemo(() => {
    ...
    if (optimalStudyTimes.length > 0) {
      recs.push(
        `最も効率が良いのは ${optimalStudyTimes[0]} です。可能であればこの時間帯に学習を集中させましょう。`
      );
    }

    // 弱点科目に関する提案
    if (subjectPerformances.length > 0) {
      const weakestSubject = [...subjectPerformances].sort(
        (a, b) => a.efficiency - b.efficiency
      )[0];
      if (weakestSubject) {
        recs.push(
          `${weakestSubject.name} の効率が最も低くなっています。この科目により多くの時間を割きましょう。`
        );
      }
      ...
      if (neglectedSubjects.length > 0) {
        recs.push(
          `${neglectedSubjects.map((s) => s.name).join('、')} は1週間以上学習していません。復習を推奨します。`
        );
      }
    }
    ...
    if (strengthsAndWeaknesses.weaknesses.length > 0) {
      recs.push(
        `最も改善が必要な分野: ${strengthsAndWeaknesses.weaknesses
          .slice(0, 3)
          .map((w) => w.name)
          .join('、')}`
      );
        ...
        {activeTab === 0 ? (
          <Grid container spacing={{ xs: 1, sm: 2, md: 3 }} alignItems="stretch">
            ...
            <Grid item xs={12} sm={6} md={3}>
              <InfoSection title="総学習時間" icon={ICONS.time} importance="primary">
                <Typography
                  variant="h4"
                  sx={{
              ...
              >
                ...
                <Box sx={CHART_HEIGHT}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={timeOfDayEfficiency} margin={getChartMargin}>
                      ...
                      />
                      <YAxis domain={[0, 100]} tick={{ fontSize: isSafariOrIOS ? 10 : 12 }} />
                      <RechartsTooltip
                ...
                <Typography variant="body2" sx={{ mt: 2 }}>
                  最適な学習時間帯:{' '}
                  {optimalStudyTimes.length > 0 ? optimalStudyTimes.join('、') : 'データ不足'}
                </Typography>
              </InfoSection>
            </Grid>
              ...
              >
                ...
                <Box sx={CHART_HEIGHT}>
                  {subjectEfficiency.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart outerRadius={90} data={subjectEfficiency} margin={getChartMargin}>
                    ...
                    >
                      <Typography variant="body2">表示するデータがありません</Typography>
                    </Box>
                  )}
                </Box>
              </InfoSection>
            ...
            {/* 弱点と強み */}
            <Grid item xs={12} sm={6} md={3}>
              <InfoSection title="強み・弱点" icon={ICONS.strength} bordered importance="primary">
                <List dense>
            ...
            {/* 弱点と強み */}
            <Grid item xs={12} sm={6} md={3}>
              <InfoSection title="強み・弱点" icon={ICONS.weakness} bordered importance="primary">
                <List dense>
