...
import { ExamInfo, ReportInfo } from '../../../../domain/entities/Dashboard';
      ...
      <Grid item xs={6} sm={3}>
        <Box
          sx={{
            ...
            ...(stats.urgentCount > 0 && {
              bgcolor:
                theme.palette.mode === 'dark' ? 'rgba(255, 50, 50, 0.2)' : 'rgba(255, 50, 50, 0.1)',
            }),
          }}
        >
          <Typography variant="caption" color="text.secondary">
...
// displayNameを設定
ReportChip.displayName = 'ReportChip';

// グループ化されたカウントダウンアイテムをメモ化
const GroupedCountdownItem = React.memo(
  ({ date, items }: { date: Date; items: CountdownData[] }) => {
    const theme = useTheme();
    const isReport = items[0].isReport;
    const [expanded, setExpanded] = useState(
      items.length <= 3 || items.some((item) => item.remainingDays <= 3)
    );
      ...
      >
        ...
        <CardContent sx={{ flexGrow: 1, pb: 1 }}>
          ...
          <Box>
            ...
            >
              <Typography variant="subtitle2">科目 ({items.length})</Typography>
              {items.length > 3 && (
                <IconButton
                  size="small"
            ...
            <Box sx={{ maxHeight: expanded ? '300px' : '150px', overflowY: 'auto', pr: 1 }}>
              ...
              {!expanded && items.length > 3 && (
                ...
                >
                  <Typography variant="caption">他 {items.length - 3} 件...</Typography>
                </Box>
              )}
            </Box>
          </Box>
...
// displayNameを設定
GroupedCountdownItem.displayName = 'GroupedCountdownItem';
