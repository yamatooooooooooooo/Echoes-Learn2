...
export const RecentProgressCard: React.FC<RecentProgressCardProps> = ({
  ...
  const getTimeColor = (date: Date | string | undefined) => {
    ...
    const now = new Date();
    const progressDate = typeof date === 'object' && date instanceof Date ? date : new Date(date);
                ...
                >
                      ...
                      {progress.reportProgress ? (
                        <AssignmentIcon
                          ...
                          sx={{
                            ...
                            transition: 'all 0.2s ease',
                            transform:
                              expandedProgressId === progress.id ? 'scale(1.2)' : 'scale(1)',
                          }}
                        />
                      ) : (
                        <MenuBookIcon
                    ...
                    primary={
                      <Typography
                        ...
                        sx={{
                          ...
                          color:
                            expandedProgressId === progress.id ? 'primary.main' : 'text.primary',
                        }}
                      >
                    ...
                    secondary={
                      <Box sx={{ mt: 0.5 }}>
                        ...
                        {getPageCount(progress.startPage, progress.endPage) > 0 ? (
                          ...
                          >
                            {progress.memo
                              ? `メモ: ${progress.memo.substring(0, 30)}${progress.memo.length > 30 ? '...' : ''}`
                              : '記録あり'}
                          </Typography>
                        )}
                      </Box>
                  ...
                  >
                    {getPageCount(progress.startPage, progress.endPage) > 0 ? (
                        ...
                        <Tooltip title="学習したページ数" arrow>
                          <Typography
                            ...
                            sx={{
                              ...
                              transition: 'all 0.2s ease',
                              transform:
                                expandedProgressId === progress.id ? 'scale(1.1)' : 'scale(1)',
                            }}
                          >
                      ...
                      <Tooltip
                        title={`満足度: ${getSatisfactionText(progress.satisfactionLevel)}`}
                        arrow
                      >
                ...
                <Collapse in={expandedProgressId === progress.id} timeout="auto" unmountOnExit>
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                      ...
                      <Typography variant="subtitle2">学習詳細</Typography>
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mt: 1 }}>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', minWidth: '160px' }}>
                        ...
                        <Typography variant="body2">
                          <strong>記録日:</strong>{' '}
                          {formatDate(progress.recordDate || progress.createdAt || new Date())}
                        </Typography>
                      </Box>
                      ...
                      {progress.satisfactionLevel && (
                        <Box sx={{ display: 'flex', alignItems: 'center', minWidth: '160px' }}>
                          ...
                          <Typography variant="body2">
                            <strong>満足度:</strong>{' '}
                            {getSatisfactionText(progress.satisfactionLevel)}
                          </Typography>
                        </Box>
                      )}
                    ...
                    {getPageCount(progress.startPage, progress.endPage) > 0 && (
                      ...
                      >
                        ...
                        <Typography variant="body2" sx={{ fontWeight: 500 }}>
                          {progress.startPage}ページから{progress.endPage}ページまで （合計:{' '}
                          {getPageCount(progress.startPage, progress.endPage)}ページ）を学習しました
                        </Typography>
                      </Box>
                    )}
