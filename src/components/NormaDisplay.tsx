...
const NormaDisplay: React.FC<NormaDisplayProps> = ({ userId }) => {
  ...
  useEffect(() => {
    if (userData && subjectProgress && dailyLearningData && weeklyLearningData) {
      const calculatedNormaData = calculateNorma(userData, subjectProgress, dailyLearningData, weeklyLearningData);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 2, backgroundColor: '#fff', borderRadius: '4px', boxShadow: '0 1px 3px rgba(0,0,0,0.12)' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
              <Typography variant="body1" sx={{ mr: 1 }}>タスク進捗:</Typography>
              <LinearProgress
                variant="determinate"
                value={
                  normaData.dailyTaskNormaBySubject.reduce((sum: number, sub: any) => sum + sub.dailyTaskNorma, 0) === 0 ? 100 :
                  Math.min(100,
                  Math.round(
                  (normaData.todayTaskProgress.reduce((sum: number, sub: any) => sum + sub.taskProgress, 0) /
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
              <Typography variant="body1" sx={{ mr: 1 }}>学習時間進捗:</Typography>
              <LinearProgress
                variant="determinate"
                value={
                  normaData.dailyTimeNormaBySubject.reduce((sum: number, sub: any) => sum + sub.dailyTimeNorma, 0) === 0 ? 100 :
                  Math.min(100,
                  Math.round(
                  (normaData.todayTimeProgress.reduce((sum: number, sub: any) => sum + sub.timeProgress, 0) /
            ...
            {normaData.dailyTaskNormaBySubject.map((subjectNorma: any, index: number) => (
              <Box key={subjectNorma.subjectId} sx={{ mb: 1, p: 1, borderRadius: '4px', backgroundColor: '#f9f9f9' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <Typography variant="caption" sx={{whiteSpace: 'nowrap'}}>
                    ({dailyLearningData.filter(data => data.subjectId === subjectNorma.subjectId).reduce((sum, data) => sum + data.completedTasks, 0)} / {subjectNorma.dailyTaskNorma})
                  </Typography>
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  <Typography variant="caption" sx={{ mr: 1 }}>
                  ...
                  />
                  <Typography variant="caption" sx={{whiteSpace: 'nowrap'}}>
                  ({dailyLearningData.filter(data => data.subjectId === subjectNorma.subjectId).reduce((sum, data) => sum + data.learningTime, 0)}分 / {subjectNorma.dailyTimeNorma}分)
                  </Typography>
                </Box>
              </Box>
            ))}
          ...
          <Paper sx={{ p: 2, backgroundColor: '#fff', borderRadius: '4px', boxShadow: '0 1px 3px rgba(0,0,0,0.12)' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
              ...
              <LinearProgress
                variant="determinate"
                value={
                  normaData.weeklyTaskNormaBySubject.reduce((sum: number, sub: any) => sum + sub.weeklyTaskNorma, 0) === 0 ? 100:
                  Math.min(100,
                  Math.round(
                  (normaData.weekTaskProgress.reduce((sum: number, sub: any) => sum + sub.taskProgress, 0) /
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
              ...
              <LinearProgress
                variant="determinate"
                value={
                  normaData.weeklyTimeNormaBySubject.reduce((sum: number, sub: any) => sum + sub.weeklyTimeNorma, 0) === 0 ? 100 :
                  Math.min(100,
                  Math.round(
                  (normaData.weekTimeProgress.reduce((sum: number, sub: any) => sum + sub.timeProgress, 0) /
            ...
            {normaData.weeklyTaskNormaBySubject.map((subjectNorma: any, index: number) => (
              <Box key={subjectNorma.subjectId} sx={{ mb: 1, p: 1, borderRadius: '4px', backgroundColor: '#f9f9f9' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  />
                  <Typography variant="caption" sx={{whiteSpace: 'nowrap'}}>
                  ({weeklyLearningData.filter(data => data.subjectId === subjectNorma.subjectId).reduce((sum, data) => sum + data.completedTasks, 0)} / {subjectNorma.weeklyTaskNorma})
                  </Typography>
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  <Typography variant="caption" sx={{ mr: 1 }}>
                  ...
                  />
                   <Typography variant="caption" sx={{whiteSpace: 'nowrap'}}>
                  ({weeklyLearningData.filter(data => data.subjectId === subjectNorma.subjectId).reduce((sum, data) => sum + data.learningTime, 0)}分 / {subjectNorma.weeklyTimeNorma}分)
                  </Typography>
                </Box>
              </Box>
            ))}
